...
export function AttendanceHistory() {
  ...
  const exportCsv = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `attendance_${new Date().toISOString().slice(0, 10)}.csv`; a.click();
            ...
            : (
              <table className="w-full text-sm">
                <thead><tr className="border-b border-slate-800 text-slate-400 text-xs">
                  <th className="text-left px-4 py-3 font-medium">Date</th><th className="text-left px-4 py-3 font-medium">Type</th>
                  <th className="text-left px-4 py-3 font-medium">Status</th><th className="text-left px-4 py-3 font-medium hidden md:table-cell">Verification</th>
                  <th className="text-left px-4 py-3 font-medium">Face</th><th className="text-left px-4 py-3 font-medium">Liveness</th><th className="text-left px-4 py-3 font-medium">Time</th>
                </tr></thead>
                <tbody>
                  {logs.map((l) => (
                    <tr key={l.id} className="border-b border-slate-800/50 last:border-0">
                      ...
                      <td className="px-4 py-3 text-slate-400 capitalize">{l.check_type.replace('_', ' ')}</td>
                      <td className="px-4 py-3 text-slate-400 capitalize">{l.status}</td>
                      <td className="px-4 py-3 text-slate-400 hidden md:table-cell capitalize">{l.verification_status}</td>
