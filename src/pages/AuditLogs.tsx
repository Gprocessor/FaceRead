...
interface AuditRow {
  id: string; action: string; entity_type: string | null; actor_role: string | null;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">System-wide audit trail of all sensitive actions.</p>
      </div>
      <div className="rounded-xl border border-slate-800 bg-slate-900 overflow-hidden">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm text-slate-500">No audit logs recorded yet</div>
        ) : (
            ...
            <tbody>
              {logs.map((l) => (
                <tr key={l.id} className="border-b border-slate-800/50 last:border-0">
                  ...
                  <td className="px-4 py-3 text-slate-400 hidden md:table-cell">{l.ip_address ?? '—'}</td>
