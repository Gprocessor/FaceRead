"""Attendance check-in / check-out / history / reports routes."""
import json
...
async def _process(request, check_type, employee_id, image, device_info, latitude, longitude):
    ...
    if should_reject_duplicate(settings, check_type):
        ...
        if existing:
            return {"success": False, "employee_id": employee_id, "attendance_log_id": existing["id"],
                    "attendance_session_id": "", "check_type": check_type, "status": existing["status"],
                    "verification_status": "rejected", "face_match_score": 0.0, "liveness_score": 0.0,
                    "message": f"Duplicate {check_type.replace('_', '-')} detected for today", "duplicate": True}
    ...
    if not v["ok"]:
        sb.table("attendance_logs").insert({
            "organization_id": org_id, "employee_id": employee_id, "attendance_date": date.today().isoformat(),
            "check_type": check_type, "status": "failed_verification", "verification_status": "failed",
            "rejection_reason": v["error"], "device_info": json.loads(device_info) if device_info else {},
            "location_latitude": float(latitude) if latitude else None,
            "location_longitude": float(longitude) if longitude else None,
        }).execute()
        return {"success": False, "employee_id": employee_id, "attendance_log_id": "", "attendance_session_id": "",
                "check_type": check_type, "status": "failed_verification", "verification_status": "failed",
                "face_match_score": 0.0, "liveness_score": 0.0, "message": v["error"], "duplicate": False}
    ...
    log_data = {
        "organization_id": org_id, "employee_id": employee_id, "attendance_date": date.today().isoformat(),
        "check_type": check_type, "status": status_val, "face_match_score": fr["score"],
        "liveness_score": liveness_score, "verification_status": verification_val,
        "face_profile_id": fr.get("face_profile_id"), "device_info": json.loads(device_info) if device_info else {},
        "location_latitude": float(latitude) if latitude else None,
        "location_longitude": float(longitude) if longitude else None,
    }
    ...
    if verification_val == "verified":
        ...
        session_id = create_attendance_session(
            org_id, employee_id, date.today(), status_val,
            now if check_type == "check_in" else None, now if check_type == "check_out" else None)
        if session_id:
    ...
    log_audit(org_id, profile["user_id"], profile["role"], f"attendance_{check_type}",
              "attendance_log", log_id,
              {"employee_id": employee_id, "status": status_val, "face_score": fr["score"], "liveness_score": liveness_score})

    return {"success": verification_val == "verified", "employee_id": employee_id, "attendance_log_id": log_id,
            "attendance_session_id": session_id, "check_type": check_type, "status": status_val,
            "verification_status": verification_val, "face_match_score": fr["score"], "liveness_score": liveness_score,
            "message": "Attendance recorded" if verification_val == "verified" else "Verification failed", "duplicate": False}


@router.post("/api/attendance/check-in")
async def check_in(request: Request, employee_id: str = Form(...), image: UploadFile = File(...),
                   liveness_session_id: str = Form(...), device_info: str = Form("{}"),
                   latitude: str | None = Form(None), longitude: str | None = Form(None)):
...
async def check_out(request: Request, employee_id: str = Form(...), image: UploadFile = File(...),
                    liveness_session_id: str = Form(...), device_info: str = Form("{}"),
                    latitude: str | None = Form(None), longitude: str | None = Form(None)):
...
async def get_history(request: Request, employee_id: str | None = Query(None),
    ...
    if date_from:
        q = q.gte("attendance_date", date_from)
    if date_to:
        q = q.lte("attendance_date", date_to)
...
async def get_reports(request: Request, date_from: str | None = Query(None), date_to: str | None = Query(None)):
    ...
    for s in sessions:
        ...
        records.append({"employee_id": s["employee_id"], "employee_name": d.get("full_name", "Unknown"),
                        "employee_code": d.get("employee_code", ""), "department": dept, "status": s["status"],
                        "check_in_time": s.get("check_in_time"), "check_out_time": s.get("check_out_time"),
                        "face_match_score": None, "liveness_score": None})
    return {"total_employees": total, "present_today": present, "late_today": late,
            "absent_today": total - present - late, "failed_verification_today": failed,
            "attendance_rate": present / total if total > 0 else 0, "records": records}
