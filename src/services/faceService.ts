...
export async function enrollFace(employeeId: string, imageBlob: Blob, consentId: string, deviceInfo: Record<string, unknown>): Promise<EnrollmentResult> {
  ...
  fd.append('employee_id', employeeId); fd.append('image', imageBlob, 'enrollment.jpg'); fd.append('consent_id', consentId); fd.append('device_info', JSON.stringify(deviceInfo));
