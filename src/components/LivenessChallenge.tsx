  ...
  if (phase === 'done' && result) {
    ...
    return (
      <div className="space-y-4 text-center">
        <Icon className={`w-14 h-14 mx-auto ${result.passed ? 'text-success' : 'text-destructive'}`} />
        <h3 className="text-lg font-semibold text-display">{result.passed ? 'Liveness Verified' : 'Verification Failed'}</h3>
        <p className="text-sm text-muted-foreground">Score: {(result.liveness_score * 100).toFixed(1)}%</p>
        {result.failure_reason && <p className="text-sm text-destructive">{result.failure_reason}</p>}
        <p className="text-xs text-muted-foreground">{result.frame_count} frames analyzed in {result.processing_time_ms}ms</p>
        {result.passed ? <Button onClick={handleDone}>Continue</Button> : <Button variant="secondary" onClick={startChallenge}>Try Again</Button>}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-lg bg-accent flex items-center justify-center"><Icon className="w-5 h-5 text-primary" /></div>
        <div><p className="text-xs uppercase tracking-wide text-muted-foreground">Liveness Check</p><h3 className="text-lg font-semibold text-display">{meta?.instruction ?? 'Preparing challenge…'}</h3></div>
      </div>
      <p className="text-sm text-muted-foreground">{meta?.description}</p>
      <CameraCapture videoRef={camera.videoRef} ready={camera.ready} error={camera.error}
        overlay={<div className="absolute inset-0 flex items-center justify-center pointer-events-none"><div className="w-40 h-52 border-2 border-primary/70 rounded-[50%]" /></div>} />
      ...
      <div className="flex gap-2">
        {!camera.ready && !camera.error && <Button onClick={() => camera.startCamera()}><ScanFace className="w-4 h-4" />Start Camera</Button>}
        {camera.ready && challenge && (
          <Button onClick={handleCaptureAndSubmit} disabled={capturing || phase === 'analyzing'}>
