          ...
          <>
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900 overflow-hidden">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-slate-800 text-slate-400 text-xs"><th className="text-left px-4 py-3 font-medium">Employee</th><th className="text-left px-4 py-3 font-medium">Code</th><th className="text-left px-4 py-3 font-medium hidden md:table-cell">Dept</th><th className="text-left px-4 py-3 font-medium">Status</th><th className="text-left px-4 py-3 font-medium">In</th><th className="text-left px-4 py-3 font-medium">Out</th><th className="text-left px-4 py-3 font-medium">Face</th><th className="text-left px-4 py-3 font-medium">Live</th></tr></thead>
                <tbody>
                  {report.records.length === 0 ? <tr><td colSpan={8} className="px-4 py-8 text-center text-slate-500">No records for the selected period.</td></tr>
                    : report.records.map((r, i) => (<tr key={i} className="border-b border-slate-800/50 last:border-0"><td className="px-4 py-3 text-slate-200">{r.employee_name}</td><td className="px-4 py-3 text-slate-400 font-mono text-xs">{r.employee_code}</td><td className="px-4 py-3 text-slate-400 hidden md:table-cell">{r.department ?? '—'}</td><td className="px-4 py-3 text-slate-400 capitalize">{r.status}</td><td className="px-4 py-3 text-slate-400">{r.check_in_time ? formatTime(r.check_in_time) : '—'}</td><td className="px-4 py-3 text-slate-400">{r.check_out_time ? formatTime(r.check_out_time) : '—'}</td><td className="px-4 py-3 text-slate-400">{formatScore(r.face_match_score)}</td><td className="px-4 py-3 text-slate-400">{formatScore(r.liveness_score)}</td></tr>))}
                </tbody>
              </table>
            </div>
          </>
