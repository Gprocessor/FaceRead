...
interface LivenessChallengeProps { onComplete: (sessionId: string) => void; }
  ...
  if (phase === 'done' && result) {
    ...
    return (
      <div className="space-y-4 text-center">
        <Icon className={`w-14 h-14 mx-auto ${result.passed ? 'text-emerald-400' : 'text-rose-400'}`} />
        <h3 className="text-lg font-semibold text-slate-100">{result.passed ? 'Liveness Verified' : 'Verification Failed'}</h3>
        ...
        <p className="text-xs text-slate-600">{result.frame_count} frames analyzed in {result.processing_time_ms}ms</p>
        {result.passed
          ? <button onClick={handleDone} className="px-5 py-2.5 rounded-lg bg-emerald-500 text-slate-950 font-medium text-sm hover:bg-emerald-400 transition-colors">Continue</button>
          : <button onClick={startChallenge} className="px-5 py-2.5 rounded-lg bg-slate-800 text-slate-300 font-medium text-sm hover:bg-slate-700 transition-colors">Try Again</button>}
      </div>
    );
  }

  const meta = challenge ? META[challenge.challenge_type] : null;
  const Icon = meta?.icon ?? ScanFace;

  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-lg bg-sky-500/10 flex items-center justify-center"><Icon className="w-5 h-5 text-sky-400" /></div>
        <div>
      ...
      <CameraCapture videoRef={camera.videoRef} ready={camera.ready} error={camera.error}
        overlay={<div className="absolute inset-0 flex items-center justify-center pointer-events-none"><div className="w-40 h-52 border-2 border-sky-400/60 rounded-[50%]" /></div>} />
      {error && <div className="flex items-center gap-2 text-sm text-rose-400 bg-rose-500/10 rounded-lg p-3"><XCircle className="w-4 h-4" />{error}</div>}
      <div className="flex gap-2">
        ...
        {camera.ready && challenge && (
          <button onClick={handleCaptureAndSubmit} disabled={capturing || phase === 'analyzing'} className="px-5 py-2.5 rounded-lg bg-sky-500 text-slate-950 font-medium text-sm hover:bg-sky-400 disabled:opacity-50 transition-colors flex items-center gap-2">
            {capturing || phase === 'analyzing' ? <><Loader2 className="w-4 h-4 animate-spin" />{capturing ? 'Capturing…' : 'Analyzing…'}</> : <><ScanFace className="w-4 h-4" />Capture &amp; Verify</>}
          </button>
        )}
      </div>
    </div>
