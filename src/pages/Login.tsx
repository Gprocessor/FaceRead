  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        // Sign-up provisions a BRAND-NEW organization (org + settings + a
        // default department) with this user as its org_admin. The database
        // trigger handle_new_user reads organization_name from metadata.
        ...
        if (err) throw err;
        if (!data.session) {
          setInfo('Organization created. Check your email to confirm, then sign in.');
          setMode('login');
          setLoading(false);
          return;
        }
    ...
    <div className="grid-blueprint min-h-screen flex items-center justify-center px-4 py-12 bg-background">
      <div className="w-full max-w-md">
        ...
        <div className="mb-8 flex flex-col items-center gap-2 text-center">
          ...
          <h1 className="text-display text-2xl font-bold">FaceAttend Admin</h1>
          <p className="text-sm text-muted-foreground">
            {mode === 'login' ? 'Sign in to manage your organization' : 'Register a new organization'}
          </p>
        </div>
        ...
        <div className="surface-panel p-6">
          <div className="flex gap-1 p-1 bg-muted rounded-lg mb-6">
            ...
            <button onClick={() => setMode('signup')} className={`flex-1 py-2 text-sm font-medium rounded-md transition-colors ${mode === 'signup' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground'}`}>Register Organization</button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'signup' && (
              <>
                <div className="space-y-1.5">
                  ...
                  <div className="relative"><Building2 className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><Input id="org" className="pl-9" value={orgName} onChange={(e) => setOrgName(e.target.value)} required placeholder="Acme Manufacturing" /></div>
                </div>
                <div className="space-y-1.5">
            ...
            <Button type="submit" disabled={loading} className="w-full">
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : mode === 'login' ? 'Sign In' : 'Create Organization'}
            </Button>
          </form>

          <p className="text-[11px] text-muted-foreground text-center mt-4">
            {mode === 'signup'
              ? 'Registering creates a new organization with you as its administrator.'
              : 'Employees mark attendance at the kiosk — no login needed there.'}
          </p>
        </div>
      </div>
    </div>
