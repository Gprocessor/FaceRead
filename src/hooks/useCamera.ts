...
export function useCamera() {
  ...
  const startCamera = useCallback(async () => { setError(null); try { const s = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user', width: { ideal: 640 }, height: { ideal: 480 } }, audio: false }); streamRef.current = s; setStream(s); return s; } catch (err) { setError(err instanceof DOMException && err.name === 'NotAllowedError' ? 'Camera permission denied. Allow camera access in your browser settings.' : 'Failed to access camera. Make sure no other app is using it.'); setReady(false); return null; } }, []);
  const stopCamera = useCallback(() => { const s = streamRef.current; if (s) s.getTracks().forEach((t) => t.stop()); streamRef.current = null; setStream(null); setReady(false); if (videoRef.current) videoRef.current.srcObject = null; }, []);
  ...
  useEffect(() => () => { const s = streamRef.current; if (s) s.getTracks().forEach((t) => t.stop()); streamRef.current = null; }, []);
  const state: CameraState = { stream, error, ready };
  return { ...state, videoRef, streamRef, startCamera, stopCamera, captureFrame, captureFrames };
}
