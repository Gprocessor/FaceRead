...
export function useLiveness() {
  ...
  const startChallenge = useCallback(async () => {
    try {
      ...
      const ch = await requestLivenessChallenge();
      setChallenge(ch); setPhase('challenge');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to start liveness challenge');
    }
  }, []);
  ...
  const submitFrames = useCallback(async (frames: Blob[], deviceInfo: Record<string, unknown>) => {
    ...
    setPhase('analyzing');
    try {
      const res = await submitLivenessCheck(challenge.session_id, challenge.challenge_type, frames, deviceInfo);
      setResult(res); setPhase('done'); return res;
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Liveness check failed'); setPhase('done'); return null;
    }
  }, [challenge]);
