...
export function FaceScanner({ onCapture, title, subtitle, buttonText, successMessage, errorMessage }: {
  ...
  const handleCapture = useCallback(async () => {
    ...
    try {
      ...
      if (!blob) { setError('Failed to capture frame. Make sure the preview is visible.'); setProcessing(false); return; }
      await onCapture(blob); setDone(true);
    } catch (err) { setError(err instanceof Error ? err.message : 'Capture failed'); } finally { setProcessing(false); }
  }, [camera, onCapture]);
  const overlay = <div className="absolute inset-0 flex items-center justify-center pointer-events-none"><div className="w-40 h-52 border-2 border-primary/70 rounded-[50%]" /></div>;
  return (
    <div className="space-y-4">
      <div><h3 className="text-lg font-semibold text-display">{title}</h3>{subtitle && <p className="text-sm text-muted-foreground mt-1">{subtitle}</p>}</div>
      ...
      <div className="flex gap-2">
        ...
        {done && <Button variant="secondary" onClick={() => { setDone(false); setError(null); }}>Try Again</Button>}
      </div>
      <p className="text-[11px] text-muted-foreground">Device fingerprint: {JSON.stringify(getDeviceInfo()).slice(0, 40)}…</p>
    </div>
  );
}
