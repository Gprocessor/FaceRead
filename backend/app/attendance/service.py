...
def get_org_settings(org_id):
    sb = get_supabase()
    r = sb.table("app_settings").select("*").eq("organization_id", org_id).maybe_single().execute()
    return r.data or {"late_threshold_minutes": 15, "work_start_time": "09:00", "work_end_time": "17:00", "require_liveness": True, "allow_multiple_check_in": False, "duplicate_check_window_minutes": 60, "face_match_threshold": FACE_MATCH_THRESHOLD, "liveness_threshold": LIVENESS_THRESHOLD}
...
def determine_status(check_type, check_time, settings):
    ...
    dl = check_time.replace(hour=sh, minute=sm, second=0, microsecond=0).replace(tzinfo=None)
...
def create_attendance_session(org_id, employee_id, attendance_date, status, check_in_time=None, check_out_time=None):
    sb = get_supabase()
    ex = sb.table("attendance_sessions").select("*").eq("employee_id", employee_id).eq("attendance_date", attendance_date.isoformat()).maybe_single().execute()
    data = {"organization_id": org_id, "employee_id": employee_id, "attendance_date": attendance_date.isoformat(), "status": status}
    if check_in_time: data["check_in_time"] = check_in_time.isoformat()
    if check_out_time: data["check_out_time"] = check_out_time.isoformat()
    if ex.data:
        sid = ex.data["id"]
        ...
        return sid
    r = sb.table("attendance_sessions").insert(data).execute()
